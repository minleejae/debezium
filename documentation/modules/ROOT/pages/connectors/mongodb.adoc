...
To optimally configure and run a {prodname} MongoDB connector, it is helpful to understand the MongoDB topologies that the connector supports, how the connector uses logical names and performs snapshots and incremental snapshots, and how it streams change events to Kafka topics.
...
The {prodname} MongoDB connector can capture changes from a MongoDB replica set or a MongoDB sharded cluster.
Understanding the differences between these deployment types helps you configure the connector's connection string and capture scope correctly.
...
The connector emits a `create` event whenever a new document is inserted into a collection.
The event payload contains the full state of the new document in the `after` field, along with source metadata that identifies the replica set, database, and collection.
...
The connector emits an `update` event whenever an existing document in a collection is modified.
The content of the event payload varies based on the value of the `capture.mode` connector property, which controls whether the full document state and pre-image are included.
...
The connector emits a `delete` event whenever a document is removed from a collection.
The event payload contains neither an `after` field nor an `updateDescription` field.

The value in a _delete_ change event has the same `schema` portion as _create_ and _update_ events for the same collection.
The `payload` portion in a _delete_ event contains values that are different from _create_ and _update_ events for the same collection.
In particular, a _delete_ event contains neither an `after` value nor a `updateDescription` value.
Here is an example of a _delete_ event for a document in the `customers` collection:
...
To capture change events from MongoDB, deploy the {prodname} MongoDB connector to a Kafka Connect environment and configure it to connect to a MongoDB replica set or sharded cluster.
...
ifdef::product[]
To deploy a {prodname} MongoDB connector, use {StreamsName} to automatically create a Kafka Connect image that includes the connector plug-in.
...
The following properties are required to run the {prodname} MongoDB connector.
You must supply values for all required properties that do not have a default value.
...
The following advanced properties control connector behavior that rarely requires customization.
The default values are suitable for most environments, but you can adjust them to optimize performance, tune snapshot behavior, or refine filtering.
